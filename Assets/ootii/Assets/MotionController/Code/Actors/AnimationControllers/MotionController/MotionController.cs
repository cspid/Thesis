...
namespace com.ootii.Actors.AnimationControllers
{
    ...
    public class MotionController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Get or create the specified motion, optionally checking disabled motions.
        /// </summary>
        /// <typeparam name="T">The type of motion to create</typeparam>        
        /// <param name="rLayerIndex">The layer on which to create the motion</param>
        /// <param name="rIncludeDisabled"></param>
        /// <param name="rName"></param>
        /// <param name="rIsEnabled"></param>
        /// <returns></returns>
        public T CreateMotion<T>(int rLayerIndex, bool rIncludeDisabled, string rName = "", bool rIsEnabled = true)
            where T : MotionControllerMotion
        {
            // Need to call this first because tthe base CreateMotion<T>() does not check disabled motions when it calls GetMotion<T>()
            T lMotion = GetMotion<T>(rLayerIndex, rIncludeDisabled);
            if (lMotion == null)
            {
                lMotion = CreateMotion<T>(rLayerIndex, rName, rIsEnabled);
        ...
        /// <summary>
        /// Get or create the specified motion, setting the proved array of tags on the motion.
        /// </summary>
        /// <typeparam name="T">The type of motion to create</typeparam>  
        /// <param name="rLayerIndex">The layer on which to create the motion</param>
        /// <param name="rTags">Array of tags to set on the motion</param>        
        /// <param name="rName"></param>
        /// <param name="rIsEnabled"></param>
        /// <returns></returns>
        public T CreateMotion<T>(int rLayerIndex, string[] rTags, string rName = "", bool rIsEnabled = true)
            where T : MotionControllerMotion
        {            
            T lMotion = CreateMotion<T>(rLayerIndex, rName, rIsEnabled); 
                      
            // Add tags
            if (lMotion != null && rTags != null)
            {
                for (int i = 0; i < rTags.Length; i++) { lMotion.AddTag(rTags[i]); }
            }
            SerializeMotion(lMotion);
        ...
        /// <summary>
        /// Create a new instance of a motion; the CreateMotion<T> method checks for a motion of the same type first
        /// so we need to use this to create additional motions of the same type.
        /// 
        /// For example: the BasicWalkRunStrafe motion named "Controlled Walk" which is used by BasicInteraction.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="rLayerIndex"></param>
        /// <param name="rName"></param>
        /// <param name="rIsEnabled"></param>
        /// <returns></returns>
        public T CreateMotionInstance<T>(int rLayerIndex, string rName = "", bool rIsEnabled = true) where T : MotionControllerMotion
        {
            int lMotionIndex = -1;
            T lMotion = null;
            ...
            MotionControllerLayer lMotionLayer = MotionLayers[rLayerIndex];

            lMotionLayer.AddMotion(lMotion);
            lMotionLayer.MotionDefinitions.Add(lMotion.SerializeMotion());
            lMotionIndex = lMotionLayer.Motions.Count - 1;
            ...
            if (lMotionIndex >= 0)
            {
                if (rName.Length > 0) { lMotion.Name = rName; }
                lMotion.IsEnabled = rIsEnabled;
                lMotionLayer.MotionDefinitions[lMotionIndex] = lMotion.SerializeMotion();
            }
        ...
        /// <summary>
        /// Disables a motion on the specified layer, if it exists
        /// </summary>
        /// <typeparam name="T"></typeparam>        
        /// <param name="rLayerIndex"></param>
        public void DisableMotion<T>(int rLayerIndex) where T : MotionControllerMotion
        {
            T lMotion = GetMotion<T>(rLayerIndex);
